...
export class Combine {
    ...
    combine() {
        return new Promise((resolve, reject) => {
            const that = this
            if (!fileUtils.directoryExists(that.sourceDir))
        ...
        function getXML(that) {
            ...
            that.#spinnerMessage = `[%1] of ${that.total} - ${that.#root
                    ...
                    logUpdate(
                        that.#spinnerMessage
                            ...
                            .replace(
                                ...
                                `\n${clc.magentaBright(
                                    nextFrame(that),
                                )} ${key} - ${index + 1} of ${fileList.length
                ...
                if (global.git.enabled &&
                    (that.metadataDefinition.packageTypeIsDirectory ||
                        (that.metadataDefinition.package !== undefined &&
                            path.dirname(fileObj.fullName).split('/').pop() in that.metadataDefinition.package))
                ) {
                    if (that.metadataDefinition.package !== undefined
                        && path.dirname(fileObj.fullName).split('/').pop() in that.metadataDefinition.package) {
                        that.#desPkg.addMember(
                            that.metadataDefinition.package[path.dirname(fileObj.fullName).split('/').pop()],
