...
describe('replaceSpecialChars', () => {
	...
	it('should return non-string values as-is', () => {
		expect(replaceSpecialChars(null as unknown as string)).toBe(null)
		expect(replaceSpecialChars(123 as unknown as string)).toBe(123)
		expect(replaceSpecialChars(undefined as unknown as string)).toBe(
			undefined,
		)
	})
})
...
describe('sanitizeErrorPath', () => {
	...
	it('should return "unknown" for invalid input', () => {
		...
		expect(sanitizeErrorPath(null as unknown as string)).toBe('unknown')
		expect(sanitizeErrorPath(undefined as unknown as string)).toBe(
			'unknown',
		)
	})
