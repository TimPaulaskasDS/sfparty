...
describe('checkVersion', () => {
	...
	it(
		'should return "A newer version" if a newer version is available',
		async () => {
	...
	it(
		'should return "You are on the latest version" if the current version is the latest version',
		async () => {
			axios.get.mockResolvedValue({
	...
	it(
		'should throw a NpmNotInstalledError if npm is not installed',
		async () => {
			axios.get.mockResolvedValue({
				data: { 'dist-tags': { latest: '2.0.0' } },
			})
	...
	it(
		'should throw a PackageNotFoundError if the package is not found on the npm registry',
		async () => {
	...
	it(
		'should throw a UpdateError if an error occurs while updating the package',
		async () => {
			axios.get.mockResolvedValue({
				data: { 'dist-tags': { latest: '2.0.0' } },
			})
	...
	it(
		'should throw a UpdateError if update.status !== 0',
		async () => {
			axios.get.mockResolvedValue({
				data: { 'dist-tags': { latest: '2.0.0' } },
			})
	...
	it(
		'should log "You are on the latest version" if update flag is true and the current version is the latest version',
		async () => {
			axios.get.mockResolvedValue({
				data: { 'dist-tags': { latest: '1.0.0' } },
			})
	...
	it(
		'should log "Application updated successfully." after successful update',
		async () => {
			axios.get.mockResolvedValue({
				data: { 'dist-tags': { latest: '2.0.0' } },
			})
